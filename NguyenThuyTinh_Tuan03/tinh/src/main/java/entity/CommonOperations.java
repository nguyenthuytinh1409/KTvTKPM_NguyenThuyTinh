			...
			try {
				new VoidVisitorAdapter<Object>() {
					...
					public void visit(ClassOrInterfaceDeclaration n, Object arg) {
						...
						if (c > 'a' && c < 'z') {
							System.out.println("**************Invalid class: " + clsName + " Posistion[" + n.getBegin()
									+ " /]" + n.getEnd());
						}
					}
				}.visit(StaticJavaParser.parse(file), null);
					...
					public void visit(ClassOrInterfaceDeclaration n, Object arg) {
						...
						if (oCmnt.isEmpty()) {
							...
							if (!content.contains("@Created-date"))
								System.out.println(
										"**************Class: " + clsName + " has comment without @Created-date ");
						}
					}
				}.visit(StaticJavaParser.parse(file), null);
			} catch (Exception e) {
			...
			try {
				new VoidVisitorAdapter<Object>() {
					...
					public void visit(FieldDeclaration n, Object arg) {
						super.visit(n, arg);
						n.getChildNodes().forEach(System.out::println);
//						char c = clsName.charAt(1);
//						Optional<Comment> oCmnt = n.getComment();
//						if (c > 'a' && c < 'z') {
//							System.out.println("**************Class: " + clsName + " khong bat dau bag chu thuong");
//						}
					}
				}.visit(StaticJavaParser.parse(file), null);
			} catch (Exception e) {
	...
	public static void cau5(File projectDir) {
		new DirExplorer((level, path, file) -> path.endsWith(".java"), (level, path, file) -> {
			System.out.println(path);
			System.out.println(Strings.repeat("=", path.length()));
			try {
